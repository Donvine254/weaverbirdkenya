        ...
        <div className="grid grid-cols-2 gap-4 py-6 md:grid-cols-3">
          {industries.map(({ icon: Icon, title, subtitle, image, link }) => (
            <Link
              key={title}
              to={link}
              className="border border-[#bf9b3f] bg-maroon p-2 last:hidden md:last:block"
            >
              <div className="group relative aspect-video overflow-hidden shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                <img
                  src={image}
                  alt={title}
